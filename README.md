...
- **Plan the Loop Before You Build It** (André Lindenberg, LinkedIn, 4 juil. 2026) — The [Loop Engineering Canvas](https://github.com/alindnbrg/loop-engineering-canvas), modeled on Osterwalder's Business Model Canvas, is a nine-field, pre-code planning exercise (Goal at center; Problem framing the top; Trigger/Actions/State on the left; Limits/Control/Model & Prompt on the right; Observability along the bottom) that a team fills out together to decide *whether* a recurring problem deserves a loop at all, before writing any harness code. Its core argument: a loop looks like a bad deal under pure cost accounting (opex forever vs. capex once for bespoke software), but that's the wrong comparison — a loop isn't asked to solve one case, it's a *capability* that keeps paying out on every future occurrence, i.e. capex billed in opex-shaped installments. How much of the problem class that capability actually covers is capped by how well the **Goal**'s gate can verify new cases — the gate is the multiplier on the loop's real value, not just an end-of-run checkpoint. Four worked examples (JVM dependency upgrades via OpenRewrite, Salesforce incident triage, Cloudflare Project Glasswing vulnerability discovery, a Meta ACH-style mutation-tested test factory) all share the same shape: a deterministic system does the actual work and verification, while the model is confined to the single step that needs judgment. Takeaway: if you can't name the deterministic check the Goal field demands, you don't have a loop yet — you have a wish that a model behaves the same way twice.
- [Frontier Engineering](https://kiro.dev/topics/frontier-engineering/) — Kiro's take on the shift from writing code to architecting autonomous agent loops: decomposing tasks for agents, building codebases optimized for agent interaction, treating code as disposable, and managing attention across multiple concurrent agent runs while holding human quality bars on output 📌 Unread

### Graph Engineering

The vocabulary of AI-agent development keeps evolving fast: **Prompt Engineering** (knowing how to ask) → **Context Engineering** (controlling what the model sees) → **Loop Engineering** (systems that cycle with the agent) → **Graph Engineering** (wiring several specialized agent loops into a graph structure). Popularized by Peter Steinberger and Hamel Husain, Graph Engineering describes architectures where specialized agents — planning, execution, testing, review, verification — act as the nodes of a graph, connected by routing, state, and validation rules along the edges. This isn't entirely new: today's major "code factories" (OpenAI's Symphony, Stripe's Minions, Ramp's Inspect, WorkOS's Project Horizon) already run this way.

**The downside — comprehension debt and rising outages**: running a code factory without human intervention creates serious maintainability problems. During RL training, models are rewarded for making tests pass immediately, with no weight given to long-term maintainability. Per [Faros AI](https://www.faros.ai/) data, despite higher code throughput, production incidents are up 242.7%, bugs per developer up 54%, and PRs merged with zero human review up by a third. Addy Osmani calls this **comprehension debt**: the growing gap between how much code gets produced and how much a human can still understand. The fix isn't less automation, it's better-placed human checkpoints — product design, system architecture, type/interface definitions, and reviewing small chunks (~100 lines) instead of large batches at the end. As Osmani puts it: *"Generation is a wide mouth; verification is the bottleneck."*

Related infra pieces spotted in the wild: [Paperclip](https://github.com/paperclipai/paperclip) (logs the model, its reasoning, and approvals straight into Git history — already listed under [Harness Tools](#harness-tools)), *herdr* (lets an agent spawn other agents in the background and await their result, tmux-style), *Tau* (Hugging Face's lightweight Python framework for coding agents), *Fractal* (isolates each agent loop in its own Git worktree with strict iteration/cost limits), and *aidermacs* (brings Aider into Emacs for reviewing AI diffs with familiar tools).
...
- [Survivre à l'ère de l'IA](https://www.sfeir.dev/survivre-a-lere-de-lia/) — Time freed up by AI doesn't become rest, it becomes a hyper-productivity spiral: the same Jevons Paradox above, applied to the individual's cognitive load instead of a market (🇫🇷 article)
...
- [pxpipe](https://github.com/teamchong/pxpipe) — **Input token** reduction tool: local proxy that rewrites bulky context (system prompt, tool docs, history) into compact PNGs before each request leaves your machine — an image's token cost is fixed by pixel size, not text density, landing ~59–70% cheaper end-to-end 📌 Unread
